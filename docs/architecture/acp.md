...
| ---------------- | ----------------------------- | --------------------------------- | -------------------------------------- |
| `acpxRecordId`   | Router (`randomUUID()`)       | Disk persistence key              | `~/.acpx/sessions/{acpxRecordId}.json` |
...
Router (randomUUID)                ACP Agent (newSession response)
       │                                    │
       ▼                                    ▼
  acpxRecordId ─── filename ──► ~/.acpx/sessions/{id}.json
                                    contains:
...
User clicks Connect
  → useAcpConnect.connect(agentId, cwd)
    ...
    → RPC: newSession({ connectionId, cwd })
      ...
      → build SessionRecord, writeSessionRecord() to ~/.acpx/sessions/
    → store.createSession(sessionId, connectionId)
    → RPC: listSessions({ connectionId })
      → listSessionsForAgent(agentCommand) reads ~/.acpx/sessions/
...
Sessions are persisted to `~/.acpx/sessions/` as JSON files. Each file is a `SessionRecord` containing metadata (no conversation history):
...
| `writeSessionRecord()`         | Persists a session record to `~/.acpx/sessions/`                  |
