...
describe("processUIMessageStream", () => {
  ...
  async function processChunks(chunks: UIMessageChunk[]) {
  ...
  describe("errors", () => {
    ...
    beforeEach(async () => {
      ...
      for (const chunk of [{ type: "error" as const, errorText: "test error" }]) {
        await processUIMessageStream({
          chunk,
          state: state!,
          write,
  ...
  describe("tool input error with dynamic flag mismatch", () => {
    ...
    it("should produce exactly one tool part (no duplicate)", async () => {
      const toolParts = state!.message.parts.filter((p: any) => p.toolCallId === "call-1");
      expect(toolParts).toHaveLength(1);
    });

    it("should keep the static tool type from tool-input-start", async () => {
      const toolPart = state!.message.parts.find((p: any) => p.toolCallId === "call-1") as any;
    ...
    describe("static tool with title", () => {
      ...
      it("should preserve title through all states", () => {
        ...
        const outputAvailableWrite = writeCalls.find((call) =>
          call.message.parts.some(
            (part) =>
              part.type === "tool-weatherTool" && (part as any).state === "output-available",
          ),
        );
        expect(
          (
