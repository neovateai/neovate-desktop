      ...
      set((state) => {
        ...
        if (part.type.startsWith("data-")) {
          switch (part.type) {
            ...
            case "data-available-models":
              storeLog(
                "appendChunk: data-available-models sid=%s count=%d",
                sessionId,
                part.data.models.length,
              );
              session.availableModels = part.data.models;
              break;

            case "data-current-model":
              storeLog(
                "appendChunk: data-current-model sid=%s model=%s",
                sessionId,
                part.data.model,
              );
              session.currentModel = part.data.model;
              break;
