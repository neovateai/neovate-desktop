...
export function EmptySessionState({ variant = "full" }: EmptySessionStateProps) {
  ...
  if (variant === "compact") {
    return <p className="px-3 py-2 text-xs text-muted-foreground">{t("session.noConversations")}</p>;
  }
  ...
  return (
    <div className="flex flex-1 items-center justify-center p-4">
      <div className="text-center">
        ...
        <p className="text-sm font-medium text-muted-foreground">{t("session.noConversationsYet")}</p>
