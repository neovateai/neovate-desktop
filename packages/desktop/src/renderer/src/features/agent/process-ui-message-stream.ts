...
// Extracted from the TransformStream inside processUIMessageStream.
// Same logic as runUpdateMessageJob's callback, called directly without streams.
...
export async function processUIMessageChunk<UI_MESSAGE extends UIMessage>({
  ...
  // ── Begin: identical to runUpdateMessageJob callback in processUIMessageStream ──
    ...
    if (part != null) {
      ...
      if (providerMetadata != null) {
        if (options.state === "output-available" || options.state === "output-error") {
          ...
          (resultPart as any).resultProviderMetadata = providerMetadata;
        } else {
          part.callProviderMetadata = providerMetadata;
        }
      }
  ...
  switch (chunk.type) {
    ...
    case "custom" as ExtendedUIMessageChunk["type"]: {
      const customChunk = chunk as Extract<ExtendedUIMessageChunk, { type: "custom" }>;
      const customPart: CustomContentUIPart = {
        type: "custom",
    ...
    default: {
      if (isDataUIMessageChunk(chunk)) {
        ...
        if (dataPartSchemas?.[chunk.type] != null) {
          const partIdx = state.message.parts.findIndex(
            (p) => "id" in p && "data" in p && (p as any).id === chunk.id && p.type === chunk.type,
          );
          const actualPartIdx = partIdx >= 0 ? partIdx : state.message.parts.length;
  ...
  // ── End: identical to runUpdateMessageJob callback ──
}
